...
class Weibo(object):
    def __init__(self, config):
        ...
        # 爬取的最大微博数，默认为所有微博 inf
        self.max_weibo_count = config.get("max_weibo_count", float('inf'))
        # 爬取的最大页数，默认为所有微博 inf
        self.max_page_count = config.get("max_page_count", float('inf'))
        ...
        try:
            ...
            if since_date <= today:    # since_date 若为未来则无需执行
                ...
                for page in tqdm(pages, desc="Progress"):
                    ...
                    # 若达到最大微博数 停止爬取
                    if self.got_count >= self.max_weibo_count:
                        logger.info("达到最大微博数限制%d，结束爬取", self.max_weibo_count)
                        break
                    # 某些user会有抓取到后面就无法抓取的情况?? 设置一个max_page_count 防止全部加载
                    if page >= self.max_page_count:
                        logger.info("已达到设定的最大抓取页数 %d", self.max_page_count)
                        break
