...
class Qwen2MLP(nn.Module):
    ...
    def forward(self, hidden_state, news_mask, user_mask):
        return self.down_proj(self.act_fn(self.gate_proj(hidden_state, news_mask, user_mask)) * self.up_proj(hidden_state, news_mask, user_mask), news_mask, user_mask)
...
class Qwen2SdpaAttention(Qwen2Attention):
    ...
    def forward(
        ...
        #query_states = self.q_proj(hidden_states)
        #key_states = self.k_proj(hidden_states)
        query_states = self.q_proj(hidden_states, news_mask=news_mask, user_mask=user_mask)
        key_states = self.k_proj(hidden_states, news_mask=news_mask, user_mask=user_mask)
        value_states = self.v_proj(hidden_states, news_mask=news_mask, user_mask=user_mask)
        ...
        attn_output = self.o_proj(attn_output, news_mask=news_mask, user_mask=user_mask)
...
class Qwen2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.mlp(hidden_states, news_mask, user_mask)
        hidden_states = residual + hidden_states
